			 ...
			 RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	/* Ajout */
	Cost rsom=cpu_run_cost + disk_run_cost;
	power_cost = count_power_cost(io_cost, cpu_cost,rsom,path->parallel_workers);
	path->io_cost = io_cost;
    path->cpu_cost = cpu_cost;
	path->power_cost = power_cost;
	/*Fin*/
}
				...
				RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	/* Ajout */
	power_cost = count_power_cost(io_cost, cpu_cost,run_cost,0);
	path->io_cost = io_cost;
        path->cpu_cost = cpu_cost;
	path->power_cost = power_cost;
	/*Fin*/
			...
			double *rows)
{
    ...
    //cpu_cost=path->subpath->total_cost - path->subpath->startup_cost;
	/* Parallel setup and communication cost. */
	startup_cost += parallel_setup_cost;
	run_cost += parallel_tuple_cost * path->path.rows;
	cpu_cost=parallel_tuple_cost * path->path.rows;
	path->path.startup_cost = startup_cost;
	path->path.total_cost = (startup_cost + run_cost);
	/* Ajout */
	power_cost = count_power_cost(io_cost, cpu_cost,run_cost,0);
	path->path.io_cost = io_cost;
        path->path.cpu_cost = cpu_cost;
	path->path.power_cost = power_cost;
	/*Fin*/
}
...
//cpu_cost = path->path.rows * comparison_cost * logN;
// cpu_cost=path->path.rows * comparison_cost;
  
	/* small cost for heap management, like cost_merge_append */
	run_cost += cpu_operator_cost * path->path.rows;
	cpu_cost += cpu_operator_cost * path->path.rows;
    //cpu_cost += cpu_operator_cost * path->path.rows;
	/*
		   ...
		   bool partial_path)
{
    ...
    cpu_cost = cpu_run_cost;
	run_cost += cpu_run_cost;

	path->path.startup_cost = startup_cost;
	path->path.total_cost = startup_cost + run_cost;
	/* Ajout */
	power_cost = count_power_cost(io_cost, cpu_cost,run_cost, path->path.parallel_workers);
	path->path.io_cost = io_cost;
    path->path.cpu_cost = cpu_cost;
	path->path.power_cost = power_cost;
	/*Fin*/
					  ...
					  Path *bitmapqual, double loop_count)
{
	...
	path->total_cost = startup_cost + run_cost;
	/* Ajout */
	power_cost = count_power_cost(io_cost, cpu_cost,run_cost,path->parallel_workers);
	path->io_cost = io_cost;
    path->cpu_cost = cpu_cost;
	path->power_cost = power_cost;
	/*Fin*/
			 ...
			 RelOptInfo *baserel, List *tidquals, ParamPathInfo *param_info)
{
	...
	io_cost=ntuples;
	cpu_cost=cpu_per_tuple * ntuples;
...
cost_recursive_union(Path *runion, Path *nrterm, Path *rterm)
{
    ...
    cpu_cost=cpu_tuple_cost * total_rows;
	runion->startup_cost = startup_cost;
	runion->total_cost = total_cost;
	/* Ajout */
	power_cost = count_power_cost(io_cost, cpu_cost,total_cost,0);
	runion->io_cost = io_cost;
    runion->cpu_cost = cpu_cost;
	runion->power_cost = power_cost;
	/*Fin*/	
		  ...
		  double limit_tuples)
{
	...
	if (output_bytes > sort_mem_bytes)
	{
       ...
       // cpu_cost+=comparison_cost * tuples * LOG2(tuples);
		/* Disk costs */
	...
	else if (tuples > 2 * output_tuples || input_bytes > sort_mem_bytes)
	{
		...
		//cpu_cost+=comparison_cost * tuples * LOG2(2.0 * output_tuples);
	}
	else
	{
    ...
    cpu_cost= cpu_operator_cost * tuples;
				  ...
				  double tuples)
{
	...
	/* Heap creation cost */
	startup_cost += comparison_cost * N * logN;
    //cpu_cost+=comparison_cost * N * logN;
	/* Per-tuple heap maintenance cost */
	run_cost += tuples * comparison_cost * logN;
    cpu_cost= tuples * comparison_cost * logN;
	/*
	 * Also charge a small amount (arbitrarily set equal to operator cost) per
		 ...
		 double input_tuples)
{
	...
	if (aggstrategy == AGG_PLAIN)
	{
		...
		output_tuples = 1;
		cpu_cost = aggcosts->transCost.per_tuple * input_tuples; 
	}
	else if (aggstrategy == AGG_SORTED || aggstrategy == AGG_MIXED)
	{
		...
		cpu_cost= (cpu_operator_cost * numGroupCols) * input_tuples + cpu_tuple_cost * numGroups;
	}
	else
	{
		/* must be AGG_HASHED */
	...
	io_cost = 0;
	//cpu_cost += total_cost - input_total_cost;
	/* Ajout */
	power_cost = count_power_cost(io_cost, cpu_cost,total_cost,0);
	path->io_cost = io_cost;
        path->cpu_cost = cpu_cost;
		   ...
		   double input_tuples)
{
	...
	cpu_cost= total_cost - input_startup_cost;
					  ...
					  JoinPathExtraData *extra)
{
	...
	io_cost += (outer_path_rows - 1) + (inner_path->rows - 1);
	...
	/* Ajout */
	//power_cost = count_power_cost(io_cost, cpu_cost,run_cost);
	workspace->io_cost = io_cost;
    workspace->cpu_cost = cpu_cost;
	workspace->power_cost = power_cost;
	/*Fin*/	
}
					...
					JoinPathExtraData *extra)
{
	...
	double		ntuples;
    /* Ajout */
	Cost		io_cost = workspace->io_cost;
	Cost		cpu_cost = workspace->cpu_cost;
	Cost		power_cost = workspace->power_cost;
	/* FIN */
		...
		extra->inner_unique)
	{
		...
		if (has_indexed_join_quals(path))
		{
            ...
            io_cost += outer_path_rows - outer_matched_rows;
			/*
					   ...
					   JoinPathExtraData *extra)
{
	...
	io_cost += outerendsel - outerstartsel;
	...
	io_cost += innerendsel - innerstartsel;
	/*
					 ...
					 JoinPathExtraData *extra)
{
	...
	io_cost += inner_path_rows * rescanratio;}
	else{
		run_cost += bare_inner_cost;
	io_cost += io_cost * rescanratio;}
	/* CPU costs */
	...
	run_cost += merge_qual_cost.per_tuple *
		((outer_rows - outer_skip_rows) +
		 ...
		 cpu_cost += merge_qual_cost.per_tuple *
		((outer_rows - outer_skip_rows) +
		 (inner_rows - inner_skip_rows) * rescanratio);
	...
	cpu_cost += cpu_per_tuple * mergejointuples;
	...
	/* Ajout */
	power_cost += count_power_cost(io_cost, cpu_cost,run_cost,0);
	path->jpath.path.io_cost = io_cost;
    path->jpath.path.cpu_cost = cpu_cost;
	path->jpath.path.power_cost = power_cost;
	/*Fin*/	
					  ...
					  JoinPathExtraData *extra)
{
    ...
    cpu_cost += cpu_operator_cost * num_hashclauses * outer_path_rows;
	/*
	...
	if (numbatches > 1)
	{
		...
		io_cost += innerpages + (innerpages + 2 * outerpages);
	}
					...
					JoinPathExtraData *extra)
{
	...
	cpu_cost += run_cost - workspace->run_cost;
...
Cost 
count_power_cost(double io_cost, double cpu_cost, double time,int wh)
{
	Cost total_power_cost = 0.0;
	//int dp=2;
	double fcd=wh*0.0552+0.00783;
	//Cost max_power_cost = 80.0; // Watts
	if(wh!=0){

	total_power_cost = B0
					 + pow(io_cost, 1) * pow(cpu_cost, 0) * B1
					 + (pow(io_cost, 0) * pow(cpu_cost, 1) * B2 + pow(io_cost, 0) * pow(cpu_cost, 1) * fcd * B2)
					 + pow(io_cost, 2) * pow(cpu_cost, 0) * B3
					 + (pow(io_cost, 0) * pow(cpu_cost, 2) * B4 +pow(io_cost, 0) * pow(cpu_cost, 2) * fcd * B4)
					 + (pow(io_cost, 1) * pow(cpu_cost, 1) * B5 + pow(io_cost, 1) * pow(cpu_cost, 1) * fcd * B5)
					 ;
	}
	else {
		total_power_cost = B0
					 + pow(io_cost, 1) * pow(cpu_cost, 0) * B1
					 + pow(io_cost, 0)* pow(cpu_cost, 1) * B2
					 + pow(io_cost, 2) * pow(cpu_cost, 0) * B3
					 + pow(io_cost, 0)* pow(cpu_cost, 2) * B4 
					 + pow(io_cost, 1)* pow(cpu_cost, 1) * B5
					 ;
	
	}
	if (total_power_cost>80) {
		total_power_cost=45;}
	//elog(LOG, "io_cost: %f\tcpu_cost: %f\ttime: %f\ttotal_power_cost: %f\n", io_cost, cpu_cost, time, total_power_cost);
	//total_power_cost = (total_power_cost > max_power_cost) ? 15 : total_power_cost;
	//total_power_cost = (total_power_cost < 0) ? 15 : total_power_cost;
	//total_power_cost *= time;
