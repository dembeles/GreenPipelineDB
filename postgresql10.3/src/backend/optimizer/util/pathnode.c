...
double		time_weight = DEFAULT_TIME_WEIGHT; // Time weight
double		power_weight = DEFAULT_POWER_WEIGHT; // Power weight
typedef enum
{
...
compare_path_costs(Path *path1, Path *path2, CostSelector criterion)
{
	...
	Cost total_cost1 = time_weight * log(path1->total_cost) + 
			   power_weight * log(path1->power_cost);
	Cost total_cost2 = time_weight * log(path2->total_cost) + 
			   power_weight * log(path2->power_cost);
	
	
	//ereport(LOG,
        //        (errmsg("compare_path_costs:\ttotal_cost_p1: %f\tpower_cost_p1: %f\ttotal_cost_p2: %f\tpower_cost_p2: %f\n", 
	//			path1->total_cost, path1->power_cost, path2->total_cost, path2->power_cost),
        //        errhidestmt(false)));
	ereport(LOG,
                (errmsg("compare_path_costs:\ttotal_cost_p1: %f\tpower_cost_p1: %f\ttotal_cost_p2: %f\tpower_cost_p2: %f\n", 
				path1->total_cost, path1->power_cost, path2->total_cost, path2->power_cost),
							  ...
							  double fraction)
{
	...
	ereport(LOG,
                (errmsg("compare_fractional_path_costs:\ttotal_cost_p1: %f\tpower_cost_p1: %f\ttotal_cost_p2: %f\tpower_cost_p2: %f\n", 
				path1->total_cost, path1->power_cost, path2->total_cost, path2->power_cost),
                errhidestmt(false)));
	...
	if(power_weight > 0)
	{	
		cost1 += fraction * path1->power_cost;
		cost2 += fraction * path2->power_cost;
		/*cost1 += fraction * sc_power_cost1;
		cost2 += fraction * sc_power_cost2;*/
	}
	if (cost1 < cost2)
		return -1;
	if (cost1 > cost2)
...
void 
scale_path_costs(Path *path1, Path *path2, Cost *sc_power_cost1, Cost *sc_power_cost2)
{
	Cost total_cost1 = path1->total_cost;
	Cost total_cost2 = path2->total_cost;
	Cost power_cost1 = path1->power_cost;
	Cost power_cost2 = path2->power_cost;
	
	Cost total_min = (total_cost1 > total_cost2) ? total_cost2 : total_cost1;
	Cost total_max = (total_cost1 > total_cost2) ? total_cost1 : total_cost2;
	Cost power_min = (power_cost1 > power_cost2) ? power_cost2 : power_cost1;
	Cost power_max = (power_cost1 > power_cost2) ? power_cost1 : power_cost2;
	
	Cost power_diff = power_max - power_min;
	power_diff = (power_diff == 0) ? 1 : power_diff;
	Cost total_diff = total_max - total_min;
	Cost r = total_diff / power_diff;
	
	*sc_power_cost1 = (power_cost1 - power_min) * r + total_min;
	*sc_power_cost2 = (power_cost2 - power_min) * r + total_min;
	
	elog(LOG, "scale_path_costs:\ttotal_cost_p1: %f\tsc_power_cost1: %f\ttotal_cost_p2: %f\tsc_power_cost2: %f\n",
		 total_cost1, *sc_power_cost1, total_cost2, *sc_power_cost2);
}
...
#define CONSIDER_PATH_STARTUP_COST(p)  \
	...
	Cost total_cost2 = time_weight * log(path2->total_cost) + 
			   power_weight * log(path2->power_cost);
	ereport(LOG,
                (errmsg("compare_path_costs_fuzzily:\ttotal_cost_p1: %f\tpower_cost_p1: %f\ttotal_cost_p2: %f\tpower_cost_p2: %f\n", 
				path1->total_cost, path1->power_cost, path2->total_cost, path2->power_cost),
                errhidestmt(false)));	
			   
	if (total_cost1 > total_cost2 * fuzz_factor)
	{
	...
	if (total_cost2 > total_cost1 * fuzz_factor)
	{
				   ...
				   int parallel_workers, List *partitioned_rels)
{
	...
	pathnode->path.total_cost = 0;
		pathnode->path.io_cost = 0;
	pathnode->path.cpu_cost = 0;
	pathnode->path.power_cost = 0;
	foreach(l, subpaths)
	{
		...
		//debut
		pathnode->path.io_cost += subpath->io_cost;
		pathnode->path.cpu_cost += subpath->cpu_cost;
		pathnode->path.power_cost += subpath->power_cost;
		//fin 
				   ...
				   SpecialJoinInfo *sjinfo)
{
	...
	if (sjinfo->semi_can_btree)
	{
		...
		sort_path.cpu_cost += cpu_operator_cost * rel->rows * numCols;
		sort_path.power_cost += count_power_cost(0, cpu_operator_cost * rel->rows , 0,0);
	}
	...
	if (pathnode->umethod == UNIQUE_PATH_HASH)
	{
		...
		//ajout
		pathnode->path.io_cost = agg_path.io_cost;
		pathnode->path.cpu_cost = agg_path.cpu_cost;
		pathnode->path.power_cost = agg_path.power_cost;
	}
	else
	{
		pathnode->path.startup_cost = sort_path.startup_cost;
		pathnode->path.total_cost = sort_path.total_cost;
		pathnode->path.io_cost = sort_path.io_cost;
		pathnode->path.cpu_cost = sort_path.cpu_cost;
		pathnode->path.power_cost = sort_path.power_cost;
	}
