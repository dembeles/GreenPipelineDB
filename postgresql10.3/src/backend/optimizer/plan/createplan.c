					  ...
					  List **qual, List **indexqual, List **indexECs)
{
	...
	if (IsA(bitmapqual, BitmapAndPath))
	{
	    ...
	    //ajout
		plan->io_cost = apath->path.io_cost;
		plan->cpu_cost = apath->path.cpu_cost;
		plan->power_cost = apath->path.power_cost;
	...
	else if (IsA(bitmapqual, BitmapOrPath))
	{
		...
		else
		{
			...
			//ajout
			plan->io_cost = opath->path.io_cost;
			plan->cpu_cost = opath->path.cpu_cost;
			plan->power_cost = opath->path.power_cost;
					  ...
					  MergePath *best_path)
{
	...
	//matplan->io_cost += matplan->plan_rows;
		matplan->cpu_cost += cpu_operator_cost * matplan->plan_rows;
		matplan->power_cost += count_power_cost(0, matplan->cpu_cost,0,0);
...
copy_generic_path_info(Plan *dest, Path *src)
{
	...
	dest->total_cost = src->total_cost;
		//Ajout
		dest->io_cost = src->io_cost;
		dest->cpu_cost = src->cpu_cost;
		dest->power_cost = src->power_cost;
...
label_sort_with_costsize(PlannerInfo *root, Sort *plan, double limit_tuples)
{
	...
	//ajout
	plan->plan.io_cost = sort_path.io_cost;
	plan->plan.cpu_cost = sort_path.cpu_cost;
	plan->plan.power_cost = sort_path.power_cost;
				  ...
				  Plan *subplan)
{
  ...
  //Ajout
	...
	plan->cpu_cost += cpu_tuple_cost * subplan->plan_rows;
	plan->power_cost += count_power_cost(0, cpu_tuple_cost * subplan->plan_rows,0,0);
	//Fin
	plan->targetlist = qptlist;
	plan->qual = qpqual;
	plan->lefttree = NULL;
...
materialize_finished_plan(Plan *subplan)
{
	...
	//Ajout
	matplan->io_cost = matpath.io_cost;
	matplan->cpu_cost = matpath.cpu_cost;
	matplan->power_cost = matpath.power_cost;
