			...
			ExplainState *es)
{
	...
	if (es->costs)
	{
		if (es->format == EXPLAIN_FORMAT_TEXT)
		{
			appendStringInfo(es->str, "  (cost=%.2f..%.2f..%.2f..%.2f..%.2f rows=%.0f width=%d)",
							 plan->startup_cost, plan->total_cost,
							 plan->io_cost, plan->cpu_cost, plan->power_cost,
		...
		else
		{
			...
			ExplainPropertyFloat("IO Cost", plan->io_cost, 2, es);
			ExplainPropertyFloat("CPU Cost", plan->cpu_cost, 2, es);
			ExplainPropertyFloat("Power Cost", plan->power_cost, 2, es);
